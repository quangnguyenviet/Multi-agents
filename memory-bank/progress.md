...
- [x] **[NEW] CV Processor**: Tính năng chuyển đổi CV PDF → mẫu CV chuyên nghiệp. Pipeline: pdfplumber → LLM JSON extraction → React editable form → Jinja2 HTML render → browser print PDF. Deps mới: `pdfplumber`, `jinja2`, `python-multipart`. Route `/cv-processor` mở cho mọi role.
...
- **API Backend**: Tất cả endpoint (Agents, Skills, Tools, Chat, CV) đã được tích hợp đầy đủ, không còn mock.
- **CV Processor**: Hoàn thành 3-step UI (Upload → Edit → Preview), 2 API endpoint (`/api/cv/extract`, `/api/cv/render`), HTML template A4 2 cột.
