...
- [x] **Storage backend cấu hình được (SQLite↔Postgres + Redis)**: env `DB_BACKEND`/`DATABASE_URL`/`REDIS_URL`/`BLOB_TTL`. `workflow._make_checkpointer()` (SqliteSaver | PostgresSaver+ConnectionPool). `conversation_store` chọn sqlite3/psycopg (ON CONFLICT upsert, `_run` đóng conn). `storage/blob_store.py` (Memory|Redis dict-like) cho `_pdf_store`/`_cv_docx_store` (Redis có TTL → hết rò rỉ). `requirements-prod.txt` (langgraph-checkpoint-postgres, psycopg, psycopg-pool, redis) — lazy import. docker-compose `--profile prod` (postgres+redis). **Default env trống = SQLite+in-memory = y cũ.** employees giữ SQLite.
- [x] **UI chọn cuộc hội thoại cũ (sidebar) — list/nạp lại/xóa**: `storage/conversation_store.py` (SQLite `data/conversations.db`: id, user_id, title, created_at, updated_at). Endpoints `GET /api/conversations`, `GET /api/conversations/{id}/messages` (nạp từ `chatbot.get_state`), `DELETE /api/conversations/{id}` (+`delete_thread`). `/api/chat` upsert mỗi lượt (title = câu hỏi đầu). Frontend: `App.jsx` quản lý `conversations` (fetch khi login & sau mỗi lượt, `loadConversation`, `deleteConversation`); `Sidebar.jsx` danh sách hội thoại + nút "Chat mới" (chuyển từ header) + xóa khi hover; CSS `.conv-*`.
- [x] **Chat nhớ lịch sử — checkpointer SQLite, per-conversation**: `workflow.py` compile `chatbot` với `SqliteSaver(data/checkpoints.db)`. `/api/chat` nhận `conversation_id` (Form) = `thread_id`; gọi `asyncio.to_thread(chatbot.invoke, ..., config)`. `llm_node` rework dùng history tích lũy (lưu HumanMessage, SystemMessage không lưu). Quét `__docx_id__` dừng ở HumanMessage (chỉ lượt hiện tại). Frontend: `conversationId` state + nút "Chat mới" (ChatWorkspace). Pin `langgraph-checkpoint-sqlite>=2,<3` (khớp langgraph 0.2.76; bản 3.x lỗi serialize). `data/checkpoints.db` gitignored.
...
- [x] **User Management — đăng nhập thật + CRUD**: `backend/storage/user_store.py` (Postgres, bcrypt trực tiếp, bảng `users`). API: `POST /api/auth/login`, CRUD `/api/users` (admin only). `LoginScreen.jsx` viết lại form username/password. `UserManager.jsx` (mới): bảng users + thêm/sửa/xóa. Sidebar thêm nav "Quản lý Users". Seed mặc định: `admin/admin123`, `user1/user123`. Bỏ passlib (không tương thích bcrypt>=4), dùng bcrypt trực tiếp.

## Trạng thái hiện tại
- **Workflow**: LangGraph ReAct — llm node bind **8 tools** (gồm `load_skill`), conditional edge tới ToolNode. **Checkpointer cấu hình được** (SQLite dev / PostgreSQL prod) nhớ lịch sử theo `conversation_id`.
...
- **Chat**: `POST /api/chat` (multipart Form, kèm `conversation_id`) → LangGraph (nhớ history qua checkpointer) → tool calls (nếu cần) → `{"response": text, "word_download_url": url|null}`.
